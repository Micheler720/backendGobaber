...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  constructor(appointsmentRepository, notificationsRepository, cacheProvider) {
    this.appointsmentRepository = appointsmentRepository;
  ...
  async execute({
    ...
    const cacheKey = `providers-appointments:${provider_id}:${(0, _dateFns.format)(appointmentDate, 'yyyy-M-d')}`;

    if ((0, _dateFns.isBefore)(appointmentDate, Date.now())) {
      throw new _AppError.default("You can't create  an appointment in past date.");
    }
    ...
    if ((0, _dateFns.getHours)(appointmentDate) < 8 || (0, _dateFns.getHours)(appointmentDate) > 17) {
      throw new _AppError.default("You can't create an appointment before 8am or after 17pm.");
    }

    const findApponitmentInSameDate = await this.appointsmentRepository.findByDate(appointmentDate, provider_id);
